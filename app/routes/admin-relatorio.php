...
$app->post('/admin/api/relatorio/fechamento-info', function () {
    ...
    try {
        ...
        $sobraRefeicoes = $totalPessoasAtendidas - $qtdRefeicoes;
        $sobraSenhas = $qtdRefeicoes - $refeicoesOfertadas;
...
$app->get('/admin/api/relatorio/pdf', function () {
    ...
    $totalPessoasAtendidas = (int)$rel['Total_pessoas_atendidas'] + $ruaMasculinoExtra + $ruaFemininoExtra + $pcdExtra;
    ...
    $sobraRefeicoes = isset($rel['sobra_refeicoes']) && $rel['sobra_refeicoes'] !== null
        ? (int)$rel['sobra_refeicoes']
        : ($totalPessoasAtendidas - $qtdRefeicoesServidas);
    $sobraSenhas = isset($rel['sobra_senhas']) && $rel['sobra_senhas'] !== null
        ? (int)$rel['sobra_senhas']
        : ($qtdRefeicoesServidas - $refeicoesOfertadas);
...
?>
    ...
    <body>
        ...
        <table class="bloco-info">
            ...
            <tr>
                <td><strong>Banco de dados:</strong> <?php echo htmlspecialchars($nomeBanco !== '' ? $nomeBanco : '-'); ?></td>
    ...
    if ($upload === 1) {
        ...
        if (!is_dir($dirTemp)) {
            if (!mkdir($dirTemp, 0775, true) && !is_dir($dirTemp)) {
                registrarHistoricoRelatorio($sql, array(
        ...
        try {
            ...
            $idHistorico = registrarHistoricoRelatorio($sql, array(
                'data_relatorio' => $dataSql,
                'nome_arquivo' => $nomeArquivo,
