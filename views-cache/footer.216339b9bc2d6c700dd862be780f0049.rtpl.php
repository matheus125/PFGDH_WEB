...
<script>
  ...
  // ---- ATUALIZA A TABELA ----
  function atualizarTabela(animacao = false) {
    ...
    let filtrados = usuarios.filter(u =>
      ...
      (u.cpf ?? "").toLowerCase().includes(pesquisa) ||
      (u.rg ?? "").toLowerCase().includes(pesquisa)
    );

    if (ordemColuna) {
      filtrados.sort((a, b) => {
        if ((a[ordemColuna] ?? "") < (b[ordemColuna] ?? "")) return ordemAsc ? -1 : 1;
        if ((a[ordemColuna] ?? "") > (b[ordemColuna] ?? "")) return ordemAsc ? 1 : -1;
        return 0;
      });
    }
    ...
    const paginaAtual = filtrados.slice(inicio, inicio + itensPorPagina);

    paginaAtual.forEach(usuario => {
    ...
    `;
      ...
      if (animacao) linha.classList.add("nova-linha");
      tabela.appendChild(linha);
    });

    document.getElementById("paginaAtual").innerText =
      `Página ${pagina} de ${Math.max(1, Math.ceil(filtrados.length / itensPorPagina))}`;
  }

  // ---- EVENTOS ----
  document.getElementById("pesquisaInput").addEventListener("input", atualizarTabela);
  ...
  document.getElementById("btnProximo").addEventListener("click", () => {
    if (pagina * itensPorPagina < usuarios.length) { pagina++; atualizarTabela(); }
  });

  document.querySelectorAll("#usuariosHeader th[data-col]").forEach(th => {
    th.addEventListener("click", () => {
      const coluna = th.dataset.col;
      ordemAsc = ordemColuna === coluna ? !ordemAsc : true;
      ordemColuna = coluna;
      atualizarTabela();
    });
  });
  ...
  document.getElementById("btnExcel").addEventListener("click", () => {
    let csv = "Nome;Telefone;Email;RG;CPF;Nascimento;Estado Civil;Escolaridade\n";
    usuarios.forEach(u => {
      csv += `${u.nome};${u.telefone};${u.email};${u.rg};${u.cpf};${u.data_nascimento};${u.estado_civil};${u.grau_escolaridade}\n`;
    });
  ...
  document.getElementById("btnPDF").addEventListener("click", () => {
    ...
    printWin.document.write("<h2>Lista de Usuários</h2>");
    printWin.document.write(document.querySelector(".table-responsive").innerHTML);
    printWin.print();
    printWin.close();
  });
