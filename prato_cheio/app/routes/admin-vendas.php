...
function salvarInformacoesFechamentoRelatorio($sql, $dataRef, $qtdRefeicoesServidas, $ocorrencias, $cardapio)
{
    ...
    if ($qtdRefeicoesServidas > $LIMITE_SENHAS_DIA) {
    ...
    if ($totalSenhasVendidas > $LIMITE_SENHAS_DIA) {
    ...
    return [
        ...
        'fechado' => isset($res[0]['fechado']) && $res[0]['fechado'] !== null ? (int)$res[0]['fechado'] : 0
    ];
}
...
$app->get("/admin/api/senhas/contagem", function () use ($app) {
    ...
    if ($fech && isset($fech[0])) {
        ...
        if (isset($fech[0]['limite']) && (int)$fech[0]['limite'] > 0) {
